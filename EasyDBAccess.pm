...
#===names
	...
	our $_name_mysql_ver_3='3.23';
	our $_name_mysql_ver_41='4.1';
...
#============================================
our $_dbh_attr_default = {PrintError=>0,RaiseError=>0,LongReadLen=>1048576,FetchHashKeyName=>'NAME_lc',AutoCommit=>1};
our $_mysql_conn_attrib= ['host','port','database','mysql_client_found_rows','mysql_compression','mysql_connect_timeout','mysql_read_default_file','mysql_read_default_group','mysql_socket','mysql_ssl','mysql_ssl_client_key','mysql_ssl_client_cert','mysql_ssl_ca_file','mysql_ssl_ca_path','mysql_ssl_cipher','mysql_local_infile'];
...
our $_str_func_new='new';
our $_str_func_id='id';
our $_str_func_execute='execute';
our $_str_func_select='select';
our $_str_func_select_one='select_one';
our $_str_func_select_col='select_col';
our $_str_func_select_row='select_row';
our $_str_func_select_array='select_array';

our $_str_new_param_err='only can accept one or two param';
our $_str_new_conn_err='connect to database failed';
our $_str_sql_null_err='sql string is null';
our $_str_inline_null_err='null in inline param';
our $_str_no_line='NO_LINE';
our $_str_param_err='PARAM_ERR';
our $_str_conn_err='CONN_ERR';
our $_str_prepare_err='PREPARE_ERR';
our $_str_exec_err='EXEC_ERR';
our $_str_dbh_do_err='when call $dbh->do, system return fause';
our $_str_dbh_prepare_err='when call $dbh->prepare, system return fause';
our $_str_dbh_execute_err='when call $dbh->execute, system return fause';
our $_str_dbh_fetchall_arrayref_err='when call $sth->fetchall_arrayref, system return fause, maybe u use select on a none select sql';
our $_str_dbh_fetchrow_arrayref_err_a='when call $sth->fetchrow_hashref, system return fause, maybe u use select on a none select sql';
our $_str_dbh_fetchrow_arrayref_err_b='when call $dbh->fetchrow_arrayref, system return fause, maybe u try to get one row from a result set with no row in it';
our $_str_dbh_fetchrow_hashref_err_a='when call $sth->fetchrow_hashref, system return fause, maybe u use select on a none select sql';
our $_str_dbh_fetchrow_hashref_err_b='when call $sth->fetchrow_hashref, system return fause, maybe u try to get one row from a result set with no row in it';
...
=head1 SYNOPSIS
	...
	$re=$dba->select_one('select id from person limit 0,1'); # $re=1
	($re,$err_code)=$dba->select_one('select id from person limit 0,1'); #$re=1,$err_code=0
	($re,$err_code)=$dba->select_one('select id from person2 limit 0,1');#table person2 doesn't exist, will die
	
	$dba->once();
	($re,$err_code,$err_detail)=$dba->select_one('select id from person2 limit %start_pos,%count',{start_pos=>0,count=>1});#won't die, because we have do "$dba->once()" before it 
	if($err_code==0){
		print "no error, id is $re";
	}elsif($err_code==5){#execute error
	...
	($re,$err_code)=$dba->execute('insert into person values (?,?)',[3,'Bob']);#1, affected_rows
	($re,$err_code)=$dba->select('select * from person');					#[{id=>1,name=>'tom'},{id=>2,name=>'gates'}]
	($re,$err_code)=$dba->select_array('select * from person');				#[[1,'tom'],[2,'gates']]
	($re,$err_code)=$dba->select_row('select * from person');				#{id=>1,name=>'tom'}
	($re,$err_code)=$dba->select_col('select id from person');				#[1,2]
	($re,$err_code)=$dba->select_one('select id from person');				#1, first line first column
	...
	$sid->note('hello world'); #will insert a record to note table
...
return value can be in scalar mode and array mode
	...
	#array mode
	($re,$err_code,$err_detail,$_pkg_name)=$dba->select_one('select 1'); # $re=1,$err_code=0,$err_detail=undef,$_pkg_name=EasyDBAccess
...
=head2 err_code - return database error code of last db operator
...
B<e.g>
	...
	my $filter=[qw/name age/];
	my ($str2,$ra_bind_param,$count,$str)=EasyDBAccess::build_update($filter,$param);  # $str2='name=?',$ra_bind_param=['jack'],$count=1,$str='name=?,'
	if($count>0){
	...
	my $filter=[qw/name age/];
	my ($str2,$ra_bind_param,$count,$str)=EasyDBAccess::build_update($filter,$param);  # $str2='name=?,age=?',$ra_bind_param=['jack',23],$count=1,$str='name=?,age=?,'
	if($count>0){
